    ...
    public sealed partial class ManagePage : Page
    {
        ...
        private async void CreateServerButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            var dialogContent = new StackPanel
            {
                Children =
                {
                    CreateTextBoxWithFilePicker("服务器名称", "输入服务器名称", false, "例子: 我的服务器"),
                    CreateTextBoxWithFilePicker("服务端核心位置", "选择服务端核心位置", true, "例子: C:\\Minecraft\\server.jar"),
                    CreateTextBoxWithFilePicker("启动参数", "输入启动参数", false, "例子: -Xmx1024M -Xms1024M -jar minecraft_server.jar nogui"),
                    CreateTextBoxWithFilePicker("Java地址", "选择Java地址", true, "例子: C:\\Program Files\\Java\\jdk-16\\bin\\java.exe"),
                    CreateTextBoxWithFilePicker("服务器文件地址", "选择服务器文件地址", true, "例子: C:\\Minecraft\\server")
                }
            };
            ...
            {
                var serverNameBox = (TextBox)((StackPanel)dialogContent.Children[0]).Children[0];
                var serverCorePathBox = (TextBox)((StackPanel)dialogContent.Children[1]).Children[0];
                var startArgumentsBox = (TextBox)((StackPanel)dialogContent.Children[2]).Children[0];
                var javaPathBox = (TextBox)((StackPanel)dialogContent.Children[3]).Children[0];
                var serverFilePathBox = (TextBox)((StackPanel)dialogContent.Children[4]).Children[0];
        ...
        private StackPanel CreateTextBoxWithFilePicker(string header, string placeholderText, bool isFilePicker, string exampleText)
        {
            var stackPanel = new StackPanel { Orientation = Orientation.Vertical, Margin = new Thickness(0, 20, 0, 0) };
            var textBox = new TextBox { Header = header, PlaceholderText = placeholderText, Name = header, Width = 480 };

            textBox.Tapped += async (sender, e) =>
            {
                if (isFilePicker)
                {
